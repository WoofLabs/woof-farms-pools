...
contract MasterChefV2 is Ownable, ReentrancyGuard {
    ...
    /// @notice total WOOF rate = toBurn + toRegular + toSpecial
    ...
    event EventUpdateWoofRate(uint256 burnRate, uint256 regularFarmRate, uint256 specialFarmRate);
    ...
    event UpdateBoostMultiplier(address indexed user, uint256 pid, uint256 oldMultiplier, uint256 newMultiplier);
     ...
     /// @param _WOOF The WOOF token contract address.
    ...
    /// only for WOOF distributions within MasterSwap products.
    function add(
        uint256 _allocPoint,
        IBEP20 _lpToken,
        bool _isRegular,
        ...
        // stake WOOF token will cause staked token and reward token mixed up,
        // may cause staked tokens withdraw as reward token,never do it.
        require(_lpToken != WOOF, "WOOF token can't be added to farm pools");
    ...
    /// @notice Update the given pool's WOOF allocation point. Can only be called by the owner.
    ...
    /// @notice Calculates and returns the `amount` of WOOF per block.
    /// @param _isRegular If the pool belongs to regular or special.
    function woofPerBlock(bool _isRegular) public view returns (uint256 amount) {
        if (_isRegular) {
            amount = MASTERCHEF_WOOF_PER_BLOCK.mul(woofRateToRegularFarm).div(WOOF_RATE_TOTAL_PRECISION);
        } else {
            amount = MASTERCHEF_WOOF_PER_BLOCK.mul(woofRateToSpecialFarm).div(WOOF_RATE_TOTAL_PRECISION);
        }
    }

    /// @notice Calculates and returns the `amount` of WOOF per block to burn.
    function woofPerBlockToBurn() public view returns (uint256 amount) {
        amount = MASTERCHEF_WOOF_PER_BLOCK.mul(woofRateToBurn).div(WOOF_RATE_TOTAL_PRECISION);
    }
    ...
    /// @notice Send WOOF pending for burn to `burnAdmin`.
    /// @param _withUpdate Whether call "massUpdatePools" operation.
    function burnWoof(bool _withUpdate) public onlyOwner {
        if (_withUpdate) {
            massUpdatePools();
        }

        uint256 multiplier = block.number.sub(lastBurnedBlock);
        uint256 pendingWoofToBurn = multiplier.mul(woofPerBlockToBurn());
    ...
    /// @param _burnRate The % of WOOF to burn each block.
    /// @param _regularFarmRate The % of WOOF to regular pools each block.
    /// @param _specialFarmRate The % of WOOF to special pools each block.
    /// @param _withUpdate Whether call "massUpdatePools" operation.
    function updateWoofRate(
        uint256 _burnRate,
        ...
        // burn WOOF base on old burn WOOF rate
        burnWoof(false);

        woofRateToBurn = _burnRate;
        woofRateToRegularFarm = _regularFarmRate;
        woofRateToSpecialFarm = _specialFarmRate;

        emit EventUpdateWoofRate(_burnRate, _regularFarmRate, _specialFarmRate);
    }
    ...
    /// @notice Settles, distribute the pending WOOF rewards for given user.
    /// @param _user The user address for settling rewards.
    /// @param _pid The pool id.
    /// @param _boostMultiplier The user boost multiplier in specific pool id.
    function settlependingWoof(
        address _user,
        uint256 _pid,
    ...
    /// @notice Safe Transfer WOOF.
    /// @param _to The WOOF receiver address.
